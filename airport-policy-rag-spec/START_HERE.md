...
2. Treat the user's selected Memgraph/Ollama stack and removal of the data lake as project decisions. Record the assignment's stack wording as an explicit substitution; do not silently reintroduce Chroma or cloud inference.
...
7. Enforce DGS in server-side code on every retrieval, expansion, citation, graph, trace, and agent path. Do not delegate authorization to the LLM.
8. Add real tests and recorded evidence with each capability. Run local and CI evaluation with actual services. Never invent results or mark skipped model tests as a passing full pipeline.
9. Make ordinary engineering decisions autonomously and record them. Ask only about a real blocker such as unavailable credentials, hardware restrictions, or an unknown target workspace. Do not deploy publicly or send policy data to hosted models.
10. Keep OpenSpec tasks and the rubric traceability file current. Finish with working code, accurate run instructions, actual evaluation results, a source-defect diagnosis, and the ordered submission PDF.
